...
function getIndexChartData(symbolKey, time) {
  return axiosTransformerAdvance(
    ...
    'date,preOpen,value').catch((error) => { throw new ExpressError(error); });
}
...
function getStocksChartData(securityCode, flag) {
  ...
  return axiosTransformerAdvance(
    ...
    DAILY_STOCKS_CLOSING_HEADERS, DAILY_STOCKS_HEADERS).catch((error) => { throw new ExpressError(error); });
}

function getStockInfoAndDayChartData(securityCode) {
  return axiosTransformerAdvance(DAILY_STOCKS_URL + securityCode, DAILY_STOCKS_CLOSING_HEADERS, DAILY_STOCKS_HEADERS).catch((error) => { throw new ExpressError(error); });
}

function getTopTurnOvers() {
  return axiosTransformer(TURNOVER_URL, TURNOVER_HEADERS).catch((error) => { throw new ExpressError(error); });
}

function getGainers() {
  return axiosTransformer(GAINERS_URL, GAINERS_HEADERS).catch((error) => { throw new ExpressError(error); });
}

function getLosers() {
  return axiosTransformer(LOSERS_URL, LOSERS_HEADERS).catch((error) => { throw new ExpressError(error); });
}
